...
也可以直接打开仓库的 `source/_posts/` 目录，点击 `Add file > Upload files` 上传 `.md` 文件。提交到 `main` 后，GitHub Actions 会自动把文件转换为博客文章并发布，不需要手写 Hexo 头部信息。

自动发布会完成这些工作：

- 用文章中的第一个一级标题作为文章标题；没有一级标题时使用文件名。
- 使用文件首次提交到 GitHub 的时间作为发布日期，后续修改不会改变原发布日期。
- 从第一段正文提取首页摘要。
- 根据文件名前缀设置分类：`[技术]文章名.md`、`[生活]文章名.md`、`[自创文]文章名.md`。
- 没有分类前缀的文件会进入 `未分类`，已有 YAML 头部的文章保持原样。
- 自动识别上传到 `source/uploads/images/` 且文件名相同的 Typora 本机图片引用。

推荐的上传顺序：

1. 先把文章图片上传到 `source/uploads/images/`，保留图片原文件名。
2. 在 Markdown 中使用 `![图片说明](/uploads/images/图片名.png)`；Typora 的本机绝对路径也可以由发布流程按同名图片修正。
3. 再把 `.md` 文件上传到 `source/_posts/` 并提交。
4. 等待仓库 `Actions` 页面出现绿色对勾，通常一至三分钟后即可在博客看到新文章。

这里有一个容易忽略的地方：仅上传 `.md` 而没有上传它引用的本机图片，网页无法访问电脑中的 `C:\Users\...` 路径，发布日志会给出缺图提醒。
